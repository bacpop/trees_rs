...
#[derive(Debug, Clone)]
pub struct NodeTuple(pub usize, pub Option<usize>, pub Option<usize>, pub Option<usize>, pub f64,  pub usize);
...
impl NodeTuple {
    ...
    pub fn add_child(&mut self, c: Option<usize>) -> () {
        match (self.get_lchild(), self.get_rchild()) {
            ...
            (Some(x), None) => {self.set_rchild(c)},
            (None, Some(x)) => {self.set_lchild(c)},
            (Some(x), Some(y)) => {panic!("Trying to add new child to node with 2 children already")},
        };
    }
}
...
impl<'a> Topology {
    ...
    pub fn from_newick(rjstr: String) -> Self {
        ...
        let mut nodevec: Vec<NodeTuple> = vec![NodeTuple(0, None, None, None, 0.0,  0); internal_idx + 1];
        ...
        // Update tree vec
        out.tree_vec = newick_to_vector(&out.get_newick(), out.count_leaves());
    ...
    pub fn get_newick(&self) -> String {
        ...
        let mut newick: Vec<String> = vec![String::from(";"), 
    ...
    pub fn count_leaves(&self) -> usize {
        self.nodes.iter().filter(|n| n.get_lchild().is_none() && n.get_rchild().is_none()).count()
    }
